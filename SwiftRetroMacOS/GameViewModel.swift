...
//        openPanel.allowedContentTypes = [.dynamicLibrary] // UTI for dylib
    ...
    private func loadCore(corePath: String) {
        ...
        if self.core?.load() == true {
            coreStatus = "Core Loaded: \(corePath.split(separator: "/").last ?? "")"
            coreIsLoaded = true
    ...
    private func loadRom(romPath: String) {
        ...
        if core.loadGame(romPath) {
             coreStatus = "Game Loaded: \(romPath.split(separator: "/").last ?? "")"
             print("macOS: Game loaded successfully")
             startRunLoop() // Start game loop ONLY after game is loaded
        } else {
    ...
    func startRunLoop() { /* ... CVDisplayLink setup ... */ isRunning = true }
    @objc func runFrame() { core?.runFrame() }
    func stopRunLoop() { /* ... CVDisplayLink teardown ... */ isRunning = false }
